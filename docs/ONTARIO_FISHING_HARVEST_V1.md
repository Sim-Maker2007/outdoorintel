...
Far-north inland expansion ships **1** (Hudson Bay / James Bay coast), **2** (Northwestern Ontario / Lake Nipigon tributaries / Woodland Caribou), **3** (Cochrane / Hearst / Kapuskasing north). FMZ 1 has no listed waterbody exceptions or sanctuaries; strip Drupal TOC/Previous/Next/Updated/Published crumbs from the last zone-wide Limits row. FR slugs for 1–3 are `zones-de-gestion-des-peches-{N}` (plural) on the official hub.
...
Great Lakes tributary expansion ships **9** (Lake Superior), **13** (Lake Huron Main Basin), and **14** (Georgian Bay / North Channel). FMZ 9’s new Black Bay waterbody heading is a “New” badge before `<strong>` — harvest it; do not skip attributed or badge-prefixed headings. FMZ 9 FR all-waters heading uses the official `ZGA` abbr — keep verbatim. FMZ 13 has no listed waterbody exceptions or sanctuaries on the official HTML. FMZ 14 lists three Big Sound of Parry Sound portions, Iroquois Bay, two St. Marys River blocks, and three sanctuary periods including Dawson Rock and tributary rivers — harvest every listed block.
...
**Still skipped:** Ontario hunting WMU 12 / Rainy River is never harvested. Ontario fishing FMZs **1–20** are harvested from the official HTML (complete-per-FMZ where listed equals harvested). Still do not claim “complete Ontario”.
...
`GET /api/regulations?zone=1` must still be Québec Zone 1 (`ON-1` / `jurisdiction=ON&zone=1` is Ontario FMZ 1).  
`GET /api/regulations?zone=5` must still be Québec Zone 5 (`ON-5` / `jurisdiction=ON&zone=5` is Ontario FMZ 5).  
`GET /api/regulations?zone=9` must still be Québec Zone 9 (`ON-9` / `jurisdiction=ON&zone=9` is Ontario FMZ 9).  
Unharvested Ontario FMZs **404** with the available ON list — they must not fall through to a matching Québec zone number.
...
`scripts/regs/harvest-ontario.mjs` (npm script `harvest:ontario`). Default `--zones=12,16,17,18`. Great Lakes slices: `--zones=9,13,14` and `--zones=19,20`. Far-north inland: `--zones=1,2,3`. Northwest/northeast inland: `--zones=4,5,6,7,8`.

Parse ontario.ca **h2/h3 Season/Limits** blocks, including **h4** nested seasons (FMZ 20 bass early catch-and-release / regular) and **h4** nested FR sanctuary periods (FMZ 5/8). Keep every same-name waterbody `<p><strong>` portion (Lake of the Woods, Nipigon River, Big Sound of Parry Sound). Waterbody headings may be `<strong id=…>` or a New/Nouveau badge immediately before `<strong>` (FMZ 9 Black Bay). FR waterbody names may be `h3` instead of `<p><strong>` when the strong-p pass is empty. Do not reuse Québec `parseGrid`. Harvest FR from FR HTML — never machine-translate limits. Keep raw Limits strings (`S-n` and `C-n` verbatim; never collapse to a single number). Fail a zone if zone-wide h3 count is 0.
...
Hub lists **Québec zones** and **Ontario FMZs 1–20** as separate lists and still says this is not complete Ontario. Season Intel waitlist CTA stays. ON pages link out to Fish ON-Line.
...
- Add `on9-walleye-en` / `on9-dore-fr` / `on9-black-bay`, `on13-walleye-en` / `on13-dore-fr`, `on14-walleye-en` / `on14-mcgregor-walleye` / `on14-dawson-sanctuary`
- `coverage-all-zones` filters by jurisdiction (QC complete check does not run on ON docs)
- Add `on-collide-qc12`, `on-collide-qc10`, `on-collide-qc19`, `on-collide-qc5`, `on-collide-qc8`, `on-collide-qc1`, `on-collide-qc2`, `on-collide-qc3`, `on-collide-qc9`, `on-collide-qc13`, and `on-collide-qc14`
...
`GET /api/regulations?zone=9` remains Québec Zone 9. `zone=ON-9` / `jurisdiction=ON&zone=9` is Ontario FMZ 9 2026.
