...
for (const q of questions) {
  ...
  if (c.season_intel_copy) {
    ...
    const liveFmz = '1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12, 15, 16, 17, 18, 19 and 20';
    const liveZgp = '1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12, 15, 16, 17, 18, 19 et 20';
    const thanks = readFileSync(join(REPO, 'src/pages/[lang]/season-intel/thanks.astro'), 'utf-8');
    ok(`${q.id}/en-on-fmz-now`, src.includes(`Ontario fishing FMZ ${liveFmz}`) && src.includes('Great Lakes FMZs 9, 13 and 14 are not harvested'), 'EN Season Intel must list live FMZ 1–8 plus 10–12/15–20 and only skip Great Lakes 9, 13 and 14');
    ok(`${q.id}/fr-on-zgp-now`, src.includes(`ZGP de pêche ${liveZgp}`) && src.includes('Les ZGP des Grands Lacs 9, 13 et 14 ne sont pas collectées'), 'FR Season Intel must list live ZGP 1–8 plus 10–12/15–20 and only skip Great Lakes 9, 13 and 14');
    ok(`${q.id}/en-not-stale-inland`, !src.includes('Ontario fishing FMZ 4, 5, 6, 7, 8, 10, 11, 12, 15, 16, 17, 18, 19 and 20') && !src.includes('Ontario FMZ 4/5/6/7/8/10/11/12/15/16/17/18/19/20'), 'EN Season Intel must not still omit FMZ 1–3 from the live list');
    ok(`${q.id}/fr-not-stale-inland`, !src.includes('ZGP de pêche 4, 5, 6, 7, 8, 10, 11, 12, 15, 16, 17, 18, 19 et 20') && !src.includes('pages ZGP 4/5/6/7/8/10/11/12/15/16/17/18/19/20'), 'FR Season Intel must not still omit ZGP 1–3 from the live list');
    ...
    ok(`${q.id}/en-coming-remaining`, src.includes('Great Lakes FMZs 9, 13 and 14') && !src.includes('remaining inland FMZs 1, 2 and 3'), 'EN What’s coming must name Great Lakes 9, 13 and 14 and must not still list inland 1, 2 and 3 as remaining');
    ok(`${q.id}/fr-coming-remaining`, src.includes('ZGP des Grands Lacs 9, 13 et 14') && !src.includes('ZGP intérieures restantes 1, 2 et 3'), 'FR Ce qui s’en vient must name Great Lakes 9, 13 et 14 and must not still list inland 1, 2 et 3 as remaining');
    ...
    ok(`${q.id}/thanks-fmz-links`, thanks.includes('[1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12, 15, 16, 17, 18, 19, 20]'), 'thanks page FMZ link list must include live 1–3');
    ok(`${q.id}/thanks-not-stale`, !thanks.includes('[4, 5, 6, 7, 8, 10, 11, 12, 15, 16, 17, 18, 19, 20]') && !thanks.includes('Ontario FMZ 4, 5, 6, 7, 8, 10, 11, 12, 15, 16, 17, 18, 19 and 20'), 'thanks page must not still omit FMZ 1–3');
    ok(`${q.id}/thanks-perk`, thanks.includes(`Ontario FMZ ${liveFmz}`) && thanks.includes(`ZGP ${liveZgp}`), 'thanks perk copy must list live FMZ/ZGP 1–8 plus 10–12/15–20');
    ...
    ok(`${q.id}/meta-on-fmz`, src.includes(`Ontario FMZ ${liveFmz}`) && src.includes(`ZGP ${liveZgp}`), 'EN+FR meta descriptions must list live FMZ/ZGP 1–8 plus 10–12/15–20');
    continue;
  }
